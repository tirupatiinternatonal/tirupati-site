...
') }}
                            ...
                            <div class="card-tools">
                                <a href="{{url('admin/testimonila/create')}}" class="btn btn-warning text-white btn-sm"><i
                                  ...
                                  @foreach($data as $key => $value)
                                        <tr>
                                            <td>{{$i++}}</td>
                                               ...
                                               <td>
                                                @if($value->photo )                                    
                                                <img src="{{ env('IMAGE_SHOW_PATH').'testimonila/'.$value['photo'] }}" class="img-fluid" style="width: 45%; height: 55px;" alt="{{$value->photo}}">
                                                @else
                                            ...
                                            <td class="d-flex">
                                                <a href="{{route('admin.testimonila.edit',$value->id)  }}" class=" text-success  btn-xs ml-3" title="Edit Testimonila"><i
                                                        class="fa fa-edit"></i></a>
                                            {!! Form::open(['method' => 'GET','route' => ['admin.testimonila.destroy', $value->id],'style'=>'display:inline','class'=>'sa-params'.$value->id.'']) !!}												
    ...
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            ...
            <div class="modal-header">
                <h4 class="modal-title text-white">{{ __('Delete Testimonial
 On Database') }}</h4>
            ...
            <!-- Modal body -->
            <form action="{{ url('admin/testimonila/destroy')}}" method="post">
                @csrf
