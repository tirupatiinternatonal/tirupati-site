                    ...
                    <div class="card card-outline card-orange">
                        <div class="card-header bg-primary">
                            ...
                            <div class="card-tools">
                                <a href="{{url('admin/download_center/create')}}" class="btn btn-warning text-white btn-sm"><i
                                ...
                                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline text-center">
                                  ...
                                  @foreach($data as $key => $value)
                                        <tr>
                                            ...
                                            <td>
                                                @if($value->photo )                                    
                                                <img src="{{ env('IMAGE_SHOW_PATH').'download_center/'.$value['photo'] }}" class="img-fluid" style="width: 45%; height: 55px;" alt="{{$value->photo}}">
                                                @else
                                            ...
                                            <td class="d-flex">
                                                <a href="{{route('admin.download_center.edit',$value->id)  }}" class=" text-success  btn-xs ml-3" title="Edit Download"><i
                                                        class="fa fa-edit"></i></a>
                                            {!! Form::open(['method' => 'GET','route' => ['admin.download_center.destroy', $value->id],'style'=>'display:inline','class'=>'sa-params'.$value->id.'']) !!}												
    ...
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            ...
            <div class="modal-header">
                <h4 class="modal-title text-white">{{ __('Delete downloads On Database') }}</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal">
                    <i class="fa fa-times" aria-hidden="true"></i>
                </button>
            </div>
            <!-- Modal body -->
            <form action="{{ url('admin/download_center/destroy')}}" method="post">
                @csrf
